...
#	
	insertLengths_per_read = []
	insertPos_per_read = []
	for i in range(len(refn)):
		if refn[i] == '-':
			if counting:
				insertLengths_per_read[-1] = insertLengths_per_read[-1] + 1
			else:
				insertLengths_per_read.append(1)
				insertPos_per_read.append(i)
				counting = True
		else:
			counting = False
	all_insertLengths_per_read.append(insertLengths_per_read)		
	all_insertPos_per_read.append(insertPos_per_read)		# I don't think I need this!
	...
	if(insert_length >= 6 and insert_length % 3 == 0 and "N" not in readn[insert_idxs]):
		...
		if(insert_length == insert_end - insert_start +1):
			...
			# CONVERT BEFORE-COORD (REV) BACK TO FORWARD COORD TO COMPARE TO AFTER-COORD
